...
def get_subscription(user_tg_id: int) -> List[Dict]:
    ...
    return [
        ...
        for sub in Subscription.select().where(
            Subscription.user == User.get(tg_id=user_tg_id)
        )
    ]
...
def user_is_owner(course_id: int, user_tg_id: int) -> bool:
    ...
    return (
        Course.get_or_none(id=course_id, owner=User.get(tg_id=user_tg_id))
        is not None
    )


def get_subscription(
    course_id: int, user_tg_id: int
) -> Union[Dict[str, Any], None]:
    """Получает подписку по курсу для пользователя"""

    return Subscription.get_or_none(
        course=course_id, user=User.get(tg_id=user_tg_id)
    )
...
def user_is_subscription(course_id: int, user_tg_id: int) -> bool:
    ...
    return (
        get_subscription(course_id=course_id, user_tg_id=user_tg_id)
        is not None
    )
