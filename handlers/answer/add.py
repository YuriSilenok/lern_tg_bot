...
async def input_answer_text_handler(
...
async def set_answer_is_valid_true_handler(
...
async def set_answer_is_valid_false_handler(
    callback: CallbackQuery, state: FSMContext
):
    """Обработка установки истиности варианта ответа"""
...
async def add_answer_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(text="Введите текст варианта ответа.")
